...
function aicb_default_options() {
	return array(
		...
		'indexed_post_types'           => array( 'page', 'post' ),
		...
		'calendar_data'                => array(
			'default_weekday_hours'  => array(
				'open'  => '09:00',
				'close' => '17:00',
			),
			'default_weekend_hours'  => array(
				'open'  => '10:00',
				'close' => '15:00',
			),
...
function aicb_has_secure_salts() {
	$salts = array( 'SECURE_AUTH_KEY', 'LOGGED_IN_KEY', 'AUTH_KEY', 'SECURE_AUTH_SALT' );
...
function aicb_is_valid_endpoint( $url ) {
	...
	$is_local_dev = in_array( $env, array( 'local', 'development', 'staging' ), true );
	if ( $is_local_dev ) {
		if ( defined( 'AICB_ALLOW_LOCAL_ENDPOINTS' ) && AICB_ALLOW_LOCAL_ENDPOINTS ) {
			return true;
		}
		if ( apply_filters( 'aicb_allow_local_endpoints', false ) ) {
			return true;
		}
	}
