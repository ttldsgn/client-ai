...
function aicb_enqueue_frontend() {
	...
	wp_enqueue_style( 'aicb-style', AICB_URL . 'assets/chatbot.css', array(), $css_ver );
	...
	wp_enqueue_script( 'aicb-script', AICB_URL . 'assets/chatbot.js', array(), $js_ver, true );
...
function aicb_maybe_inject() {
	if ( ! aicb_opt( 'enabled' ) || ! aicb_opt( 'show_on_all' ) ) {
		return;
	}
	if ( did_action( 'aicb_shortcode_rendered' ) ) {
		return;
	}
...
function aicb_ajax_chat() {
	...
	if ( empty( $session_id ) || false === $session_data ) {
		...
		$session_data = array( 'ip_hash' => $ip_hash );
		...
		wp_send_json_error( array( 'message' => 'Session expired or invalid.' ), 403 );
	}

	$confirm = isset( $_POST['confirm_handover'] ) && $_POST['confirm_handover'] === 'true';

	if ( strlen( $question ) < 2 || strlen( $question ) > 1000 ) {
		wp_send_json_error( array( 'message' => 'Invalid question length.' ), 400 );
	}
	...
	$custom   = $wpdb->get_row(
		$wpdb->prepare(
			"SELECT answer FROM {$qa_table} WHERE active = 1 AND (LOWER(%s) LIKE CONCAT('%%', LOWER(question), '%%') OR LOWER(question) LIKE CONCAT('%%', LOWER(%s), '%%')) LIMIT 1",
			$question,
			$question
		)
	);
	...
	if ( is_wp_error( $result ) ) {
		...
		if ( in_array( $err_code, array( 'no_key', 'no_endpoint', 'unsafe_endpoint', 'unknown_provider' ), true ) ) {
			wp_send_json_error( array( 'message' => $err_msg ), 500 );
		} else {
			error_log( 'AICB AI Provider Error: ' . $err_msg );
			wp_send_json_error( array( 'message' => 'The AI assistant could not respond at this time. Please try again later.' ), 502 );
		}
	}
...
function aicb_is_handover_requested( $question ) {
	...
	foreach ( $phrases as $phrase ) {
		if ( strpos( $question, $phrase ) !== false ) {
			return true;
		}
	}
	return false;
...
function aicb_is_positive_confirmation( $text ) {
	...
	$confirmations = array( 'yes', 'yeah', 'yep', 'sure', 'please', 'ok', 'okay', 'yes please', 'absolutely', 'do it' );
	return in_array( $text, $confirmations, true );
}

function aicb_detect_handover( $text ) {
	$text    = strtolower( $text );
	$phrases = array( 'live person', 'real person', 'human', 'representative', 'contact us', 'live agent', 'customer service' );
	foreach ( $phrases as $phrase ) {
...
function aicb_clean_url( $url ) {
	return esc_url( $url, array( 'http', 'https', 'tel', 'sms', 'mailto' ) );
}
...
function aicb_get_handover_url() {
	...
	switch ( $type ) {
		case 'whatsapp':
			return 'https://wa.me/' . preg_replace( '/[^0-9]/', '', $target );
		case 'tel':
			return 'tel:' . preg_replace( '/[^0-9+]/', '', $target );
		case 'sms':
			return 'sms:' . preg_replace( '/[^0-9+]/', '', $target );
		default:
...
function aicb_ajax_feedback() {
	...
	if ( ! aicb_opt( 'enable_feedback' ) ) {
		wp_send_json_error( array( 'message' => 'Feedback is disabled.' ), 403 );
	}

	$log_id = isset( $_POST['log_id'] ) ? (int) $_POST['log_id'] : 0;
	$rating = isset( $_POST['rating'] ) ? (int) $_POST['rating'] : -1;

	if ( ! $log_id || ! in_array( $rating, array( 0, 1 ), true ) ) {
...
function aicb_ajax_lead_submit() {
	...
	if ( ! aicb_opt( 'enable_lead_capture' ) ) {
		wp_send_json_error( array( 'message' => 'Lead capture is disabled.' ), 403 );
	}
	...
	if ( ! empty( $session_id ) ) {
		...
		if ( false === $session_data ) {
			wp_send_json_error( array( 'message' => 'Session not found or has expired.' ), 403 );
		}
		if ( ! hash_equals( $session_data['ip_hash'], $ip_hash ) ) {
			wp_send_json_error( array( 'message' => 'Unauthorized: session IP mismatch.' ), 403 );
		}
	}
	...
	if ( empty( $name ) || empty( $email ) || ! is_email( $email ) ) {
		wp_send_json_error( array( 'message' => 'Please provide a valid name and email address.' ), 400 );
	}

	if ( strlen( $name ) > 255 || strlen( $email ) > 255 ) {
		wp_send_json_error( array( 'message' => 'Invalid input length.' ), 400 );
	}
...
function aicb_ajax_export_transcript() {
	...
	if ( ! aicb_opt( 'enable_transcript_export' ) ) {
		wp_send_json_error( array( 'message' => 'Transcript export is disabled.' ), 403 );
	}
	...
	// Honeypot: if 'website' field is filled, silently reject (bot)
	if ( ! empty( $_POST['website'] ) ) {
		wp_send_json_success( array( 'message' => 'Transcript sent!' ) );
	}
	...
	if ( empty( $email ) || ! is_email( $email ) ) {
		wp_send_json_error( array( 'message' => 'Please provide a valid email address.' ), 400 );
	}

	if ( empty( $session_id ) ) {
		wp_send_json_error( array( 'message' => 'No conversation session found.' ), 400 );
	}
	...
	if ( empty( $messages ) ) {
		wp_send_json_error( array( 'message' => 'No conversation history found for this session.' ), 404 );
	}
