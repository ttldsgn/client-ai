...
<div class="wrap aicb-wrap">
	...
	<form method="post">
		...
		<div class="aicb-section">
			...
			<label>
				<input type="checkbox" name="enable_calendar_tools" value="1" <?php echo checked( $enable_tools, 1 ); ?> />
		...
		<div class="aicb-section">
			...
			<table class="form-table">
				<tr>
					...
					<td>
						Open: <input type="time" name="wd_open" value="<?php echo esc_attr( $calendar['default_weekday_hours']['open'] ?? '09:00' ); ?>" />
						Close: <input type="time" name="wd_close" value="<?php echo esc_attr( $calendar['default_weekday_hours']['close'] ?? '17:00' ); ?>" />
					</td>
				</tr>
				<tr>
					<th>Weekends (Sat–Sun)</th>
					<td>
						Open: <input type="time" name="we_open" value="<?php echo esc_attr( $calendar['default_weekend_hours']['open'] ?? '10:00' ); ?>" />
						Close: <input type="time" name="we_close" value="<?php echo esc_attr( $calendar['default_weekend_hours']['close'] ?? '15:00' ); ?>" />
						&nbsp; Status:
						<select name="we_status">
							<option value="open"   <?php echo selected( $calendar['default_weekend_status'] ?? 'closed', 'open' ); ?>>Open</option>
							<option value="closed" <?php echo selected( $calendar['default_weekend_status'] ?? 'closed', 'closed' ); ?>>Closed</option>
						</select>
					</td>
				</tr>
			</table>
	...
	<div class="aicb-section">
		<h2><?php echo $edit_entry ? 'Edit Entry' : 'Add Entry'; ?></h2>
		<form method="post">
			<?php wp_nonce_field( 'aicb_cal_action', 'aicb_cal_nonce' ); ?>
			<input type="hidden" name="aicb_action" value="<?php echo $edit_entry ? 'update_entry' : 'add_entry'; ?>" />
			<?php if ( $edit_entry ) : ?>
				<input type="hidden" name="entry_index" value="<?php echo $edit_idx; ?>" />
			<?php endif; ?>
			<table class="form-table">
				<tr>
					<th>Date</th>
					<td>
						...
						?>
						<input type="text" name="entry_date" value="<?php echo esc_attr( $display_input_val ); ?>" placeholder="MM/DD/YYYY" class="regular-text aicb-datepicker" autocomplete="off" readonly="readonly" style="cursor: pointer; background: #fff;" />
						<br>
						<label style="display:inline-block; margin-top:8px;">
							<input type="checkbox" name="entry_is_recurring" value="1" <?php echo ( $edit_entry && 0 === strpos( $edit_entry['date'] ?? '', '--' ) ) ? 'checked' : ''; ?> />
				...
				<tr>
					<th>Label</th>
					<td><input type="text" name="entry_label" value="<?php echo $edit_entry ? esc_attr( $edit_entry['label'] ) : ''; ?>" placeholder="e.g. Labor Day" class="regular-text" /></td>
				</tr>
				<tr>
					<th>Status</th>
					<td>
						<select name="entry_status">
							<option value="open"   <?php echo selected( $edit_entry['status'] ?? 'open', 'open' ); ?>>Open</option>
							<option value="closed" <?php echo selected( $edit_entry['status'] ?? 'open', 'closed' ); ?>>Closed</option>
							<option value="reduced"<?php echo selected( $edit_entry['status'] ?? 'open', 'reduced' ); ?>>Reduced Hours</option>
						</select>
					</td>
				</tr>
				<tr>
					...
					<td>
						Open: <input type="time" name="entry_hours_open" value="<?php echo $edit_entry ? esc_attr( $edit_entry['hours_open'] ?? '' ) : ''; ?>" />
						Close: <input type="time" name="entry_hours_close" value="<?php echo $edit_entry ? esc_attr( $edit_entry['hours_close'] ?? '' ) : ''; ?>" />
					</td>
				</tr>
			</table>
	...
	<div class="aicb-section" id="aicb-entry-list">
		<h2>Special Dates (<?php echo $total_entries; ?>)</h2>
		<?php if ( $total_entries > 0 ) : ?>
			...
			<div style="display:flex;gap:20px;flex-wrap:wrap;margin-bottom:16px;font-size:13px;color:#555">
				<span>📅 <strong><?php echo $fixed_cnt; ?></strong> fixed · <strong><?php echo $recur_cnt; ?></strong> recurring</span>
				<span>🔴 <strong><?php echo $closed_cnt; ?></strong> closed · 🟢 <strong><?php echo $open_cnt; ?></strong> open · 🟡 <strong><?php echo $reduced_cnt; ?></strong> reduced</span>
				<?php if ( ! empty( $years ) ) : ?>
					<span>📆 Years:
			...
			<form method="post" id="aicb-bulk-form" onsubmit="return confirm('Delete selected entries?')">
				...
				<table class="aicb-logs" id="aicb-entries-table">
					...
					<tbody>
						...
						<?php foreach ( $display_entries as $idx => $e ) : ?>
							<tr class="aicb-entry-row" data-date="<?php echo esc_attr( $e['date'] ?? '' ); ?>" data-label="<?php echo esc_attr( $e['label'] ?? '' ); ?>" data-status="<?php echo esc_attr( $e['status'] ?? 'open' ); ?>">
								<td><input type="checkbox" class="aicb-entry-cb" name="entry_indices[]" value="<?php echo $idx; ?>" /></td>
								<td><?php echo esc_html( aicb_format_date_us( $e['date'] ?? '' ) ); ?></td>
								<td><?php echo esc_html( $e['label'] ?? '' ); ?></td>
								<td><?php echo esc_html( $e['status'] ?? 'open' ); ?></td>
								<td><?php echo ( ! empty( $e['hours_open'] ) && ! empty( $e['hours_close'] ) ) ? esc_html( $e['hours_open'] . '-' . $e['hours_close'] ) : '—'; ?></td>
								<td>
									<a href="?page=ai-chatbot-calendar&edit_entry=<?php echo $idx; ?>" class="button button-small">Edit</a>
									<button type="button" class="button button-small button-link-delete" onclick="aicbSubmitSingleAction('delete_entry', <?php echo $idx; ?>, 'Delete this entry?')">Delete</button>
								</td>
							</tr>
						<?php endforeach; ?>
					</tbody>
				...
				<?php if ( $total_pages > 1 ) : ?>
					<div style="margin-top:14px" class="aicb-pagination">
						...
						if ( '' !== $filter_year ) {
							$page_url = add_query_arg( 'ek_year', $filter_year, $page_url );
						}
						?>
						<span style="margin-right:12px">Page <?php echo $paged_ek; ?> of <?php echo $total_pages; ?></span>
						<?php if ( $paged_ek > 1 ) : ?>
							<a class="button button-small" href="<?php echo esc_url( add_query_arg( 'ek_page', 1, $page_url ) ); ?>">« First</a>
							<a class="button button-small" href="<?php echo esc_url( add_query_arg( 'ek_page', $paged_ek - 1, $page_url ) ); ?>">‹ Prev</a>
						<?php endif; ?>
						<?php if ( $paged_ek < $total_pages ) : ?>
							<a class="button button-small" href="<?php echo esc_url( add_query_arg( 'ek_page', $paged_ek + 1, $page_url ) ); ?>">Next ›</a>
							<a class="button button-small" href="<?php echo esc_url( add_query_arg( 'ek_page', $total_pages, $page_url ) ); ?>">Last »</a>
						<?php endif; ?>
					</div>
				<?php endif; ?>
			</form>
		...
		<form method="post" onsubmit="return confirm('Add country holidays to the calendar for the specified year range? Existing entries will not be duplicated.');">
			...
			<table class="form-table">
				<tr>
					...
					<td>
						<input type="number" name="seed_from_year" id="seed_from_year" value="<?php echo (int) current_time( 'Y' ); ?>" min="2020" max="2099" style="width:100px" />
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="seed_to_year">To Year</label></th>
					<td>
						<input type="number" name="seed_to_year" id="seed_to_year" value="<?php echo (int) current_time( 'Y' ) + 2; ?>" min="2020" max="2099" style="width:100px" />
					</td>
				</tr>
				<tr>
