...
function aicb_admin_styles() {
	...
	if ( ! $screen || strpos( $screen->id, 'ai-chatbot' ) === false ) {
		return;
	}
	?>
	<style>
...
function aicb_admin_enqueue_scripts( $hook_suffix ) {
	if ( strpos( $hook_suffix, 'ai-chatbot-calendar' ) !== false ) {
		...
		wp_enqueue_style( 'jquery-ui-style', AICB_URL . 'assets/jquery-ui.css', array(), '1.12.1' );
	}
}
...
function aicb_register_settings() {
		...
		register_setting(
			...
			array(
				'sanitize_callback' => function ( $val ) use ( $f ) {
					return aicb_sanitize_specific_option( $val, $f ); },
			)
		);
	}
	foreach ( array_keys( aicb_get_providers() ) as $pid ) {
		register_setting(
			...
			'aicb_key_' . $pid,
			array(
				'sanitize_callback' => function ( $val ) use ( $pid ) {
					return aicb_sanitize_key_field( $val, $pid ); },
			)
		);
	}
}
...
function aicb_handle_export_settings() {
	...
	$general_option_keys = array(
		...
		'show_footer_help_button',
		'handover_type',
		'handover_target',
		'handover_btn_text',
		'contact_btn_text',
		'contact_btn_url',
		'handover_primary_text',
		'handover_secondary_bg',
		'handover_secondary_text',
		'handover_btn_radius',
		'always_show_handover_buttons',
		'business_name',
		'pronoun_perspective',
		'chatbot_tone',
		'chatbot_language_mode',
		'chatbot_language',
		'enable_feedback',
	...
	if ( $export_prompts ) {
		$prompt_keys = array( 'system_prompt', 'prompt_temporal_pivot', 'prompt_tool_instruction', 'prompt_negative_constraints' );
		$prompts     = array();
	...
	if ( $export_qa ) {
		...
		$export['data']['qa_entries'] = $qa_rows ?: array();
	}
	...
	if ( $export_models ) {
		...
		$export['data']['custom_models'] = $model_rows ?: array();
	}
...
function aicb_handle_import_settings() {
	...
	if ( isset( $import_data['general'] ) && is_array( $import_data['general'] ) ) {
		foreach ( $import_data['general'] as $key => $value ) {
			if ( ! in_array( $key, $general_option_keys, true ) ) {
				continue;
			}
	...
	$prompt_allowlist = array( 'system_prompt', 'prompt_temporal_pivot', 'prompt_tool_instruction', 'prompt_negative_constraints' );
	if ( isset( $import_data['prompts'] ) && is_array( $import_data['prompts'] ) ) {
		foreach ( $import_data['prompts'] as $key => $value ) {
			if ( ! in_array( $key, $prompt_allowlist, true ) ) {
				continue;
			}
			update_option( 'aicb_' . $key, aicb_sanitize_import_option( $value, $key ) );
		}
	...
	if ( isset( $import_data['custom_models'] ) && is_array( $import_data['custom_models'] ) ) {
		...
		$wpdb->delete( $model_table, array( 'is_custom' => 1 ), array( '%d' ) );
		$count = 0;
		foreach ( $import_data['custom_models'] as $model ) {
			if ( empty( $model['provider_id'] ) || empty( $model['model_id'] ) || empty( $model['name'] ) ) {
				continue;
			}
...
function aicb_sanitize_import_option( $val, $field ) {
	...
	$array_allowed = array( 'indexed_post_types', 'calendar_data' );
	...
	if ( $field === 'calendar_data' ) {
		...
		$sanitized                           = array();
		$wd                                  = isset( $val['default_weekday_hours'] ) && is_array( $val['default_weekday_hours'] ) ? $val['default_weekday_hours'] : array();
		$sanitized['default_weekday_hours']  = array(
			'open'  => sanitize_text_field( $wd['open'] ?? '09:00' ),
			'close' => sanitize_text_field( $wd['close'] ?? '17:00' ),
		);
		$we                                  = isset( $val['default_weekend_hours'] ) && is_array( $val['default_weekend_hours'] ) ? $val['default_weekend_hours'] : array();
		$sanitized['default_weekend_hours']  = array(
		...
		$entries                             = isset( $val['entries'] ) && is_array( $val['entries'] ) ? $val['entries'] : array();
	...
	if ( in_array( $field, array( 'handover_apology', 'handover_prompt', 'handover_btn_text', 'contact_btn_text', 'handover_primary_text', 'handover_secondary_bg', 'handover_secondary_text', 'handover_trigger_phrases' ), true ) ) {
		return sanitize_textarea_field( $val );
	}
	if ( in_array( $field, array( 'contact_btn_url', 'handover_target', 'handover_btn_radius' ), true ) ) {
		return sanitize_text_field( $val );
	}

	// Lead capture & transcript export fields
	$lead_boolean_fields = array( 'enable_lead_capture', 'enable_transcript_export', 'show_footer_help_button' );
...
function aicb_sanitize_specific_option( $val, $field ) {
	...
	if ( in_array( $field, array( 'handover_apology', 'handover_prompt', 'handover_btn_text', 'contact_btn_text', 'handover_primary_text', 'handover_secondary_bg', 'handover_secondary_text' ), true ) ) {
		return sanitize_text_field( $val );
	}
	if ( in_array( $field, array( 'contact_btn_url', 'handover_target', 'handover_btn_radius' ), true ) ) {
		return sanitize_text_field( $val );
	...
	// Lead capture & transcript export fields
	$lead_boolean_fields = array( 'enable_lead_capture', 'enable_transcript_export' );
	if ( in_array( $field, $lead_boolean_fields, true ) ) {
		return (int) ( ! empty( $val ) );
	}
	if ( $field === 'lead_notification_email' ) {
...
function aicb_handle_manual_prompt_reset() {
	if ( isset( $_POST['aicb_reset_prompts_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['aicb_reset_prompts_nonce'] ) ), 'aicb_reset_prompts' ) ) {
		...
		$prompt_keys = array( 'prompt_temporal_pivot', 'prompt_tool_instruction', 'prompt_negative_constraints', 'system_prompt' );
		foreach ( $prompt_keys as $key ) {
...
function aicb_page_calendar() {
	...
	if ( $paged_ek > $total_pages ) {
		$paged_ek = $total_pages;
	}
	...
	if ( isset( $_POST['aicb_qa_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['aicb_qa_nonce'] ) ), 'aicb_qa_action' ) ) {
		...
		if ( $action === 'add' ) {
			...
			$wpdb->update(
				...
				array( 'id' => (int) $_POST['qa_id'] ),
				array( '%s', '%s' ),
				array( '%d' )
			);
		} elseif ( $action === 'delete' && isset( $_POST['qa_id'] ) ) {
			$wpdb->delete( $table, array( 'id' => (int) $_POST['qa_id'] ), array( '%d' ) );
		} elseif ( $action === 'toggle' && isset( $_POST['qa_id'] ) ) {
			$row = $wpdb->get_row( $wpdb->prepare( "SELECT active FROM $table WHERE id=%d", (int) $_POST['qa_id'] ) );
			if ( $row ) {
				$wpdb->update( $table, array( 'active' => $row->active ? 0 : 1 ), array( 'id' => (int) $_POST['qa_id'] ), array( '%d' ), array( '%d' ) );
			}
		}
	}
	...
	if ( isset( $_POST['aicb_models_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['aicb_models_nonce'] ) ), 'aicb_models_action' ) ) {
		...
		if ( $action === 'add' || $action === 'update' ) {
			...
			if ( empty( $provider_id ) || empty( $model_id ) || empty( $name ) ) {
				...
				if ( $exists ) {
					...
					$wpdb->insert(
						...
						array( '%s', '%s', '%s', '%s', '%s', '%d', '%d', '%d', '%s', '%s', '%d', '%d', '%d' )
					);
				...
				$update_formats = array( '%s', '%s', '%s', '%s', '%d', '%d', '%d', '%s' );
				...
				$wpdb->update( $table, $update_fields, array( 'id' => $db_id ), $update_formats, array( '%d' ) );
			...
			if ( $row ) {
				...
				$wpdb->update( $table, array( 'active' => $new_active ), array( 'id' => $db_id ), array( '%d' ), array( '%d' ) );
...
function aicb_page_leads() {
	...
	if ( isset( $_POST['aicb_leads_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['aicb_leads_nonce'] ) ), 'aicb_leads_action' ) ) {
		...
		if ( $action === 'delete' && isset( $_POST['lead_id'] ) ) {
			$wpdb->delete( $table, array( 'id' => (int) $_POST['lead_id'] ), array( '%d' ) );
			echo '<div class="notice notice-success"><p>Lead deleted.</p></div>';
		} elseif ( $action === 'mark_read' && isset( $_POST['lead_id'] ) ) {
			$wpdb->update( $table, array( 'read_status' => 1 ), array( 'id' => (int) $_POST['lead_id'] ), array( '%d' ), array( '%d' ) );
			echo '<div class="notice notice-success"><p>Lead marked as read.</p></div>';
		} elseif ( $action === 'mark_unread' && isset( $_POST['lead_id'] ) ) {
			$wpdb->update( $table, array( 'read_status' => 0 ), array( 'id' => (int) $_POST['lead_id'] ), array( '%d' ), array( '%d' ) );
