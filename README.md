...
- **Lead Capture Form**: When a visitor confirms they want to speak to someone, an inline contact form appears alongside the existing handover buttons. Visitors can leave their name, email, and a message — saved directly to the database and optionally emailed to the site admin. Configurable in Settings under "Lead Capture & Transcript Export."
- **Chat Transcript Export**: A discreet "📧 Email transcript" link in the chat footer lets visitors email themselves a complete copy of their conversation. The full session history is retrieved from the logs database and sent as a formatted plain-text transcript. Spam-protected via IP-based rate limiting and a honeypot field.
